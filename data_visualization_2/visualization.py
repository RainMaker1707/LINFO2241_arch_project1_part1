...
def visualization(data:dict):
    ...
    for test in data.values():
        for i in range(len(test["run_times"])):
            df = df.append({"run_times":test["run_times"][i],"request_rate":test["request_rate"],"key_size":test["key_size"],"file_size":test["file_size"],"n_threads":test["nb_threads"],"delay":test["delay"],"loss":test["loss"]},ignore_index=True)

    fig, ax = plt.subplots(figsize=(10,5))
    # sns.violinplot(data=df,y="run_times",x="request_rate")
    # ax.set(yscale="log")
    sns.boxplot(data=df,y="run_times",x="loss", ax=ax)
    ax.set_axisbelow(True)
    plt.xlabel("Loss [%]")
    plt.ylabel("Request time [s]")
    plt.title("Request time depending on network packet loss percentage")
    plt.ylim(bottom=0)
...
if __name__ == "__main__":
    ...
    data = parsing.parse("../output/6.txt")
