...
int main(int argc, char **argv){
    ...
    while(true){
        ...
        if (file_size % key_size != 0 || key_size > file_size){
            ...
            // Encrypt file
            encrypt_file(key_size, key, file_size, my_test_file, encrypted_file);
            printf("Bytes received from client : %i\n"
                   "INDEX : %i\n"
                   "KEY_SIZE : %i\n"
                   "KEY : %s\n"
                   "FILE : %c,%c,%c,%c\n"
                   "ENCR FILE : %c,%c,%c,%c\n",nb_bytes_read, file_index, key_size, key, my_test_file[0], my_test_file[1], my_test_file[2], my_test_file[3], encrypted_file[0], encrypted_file[1], encrypted_file[2], encrypted_file[3]);
            err_code = 0;
        }
        free(buff);
        free(key);
